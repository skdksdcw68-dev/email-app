...
final class PreviewTextTests: XCTestCase {
    ...
    func testImageMarkersAreRemoved() {
        ...
        XCTAssertEqual(
            MailText.previewText(from: "[image: Google] You allowed Maily access"),
            "You allowed Maily access"
        )
    }

    func testBareImageMarkerIsRemoved() {
        XCTAssertEqual(MailText.previewText(from: "[image] Hello there"), "Hello there")
    }

    func testCidMarkersAreRemoved() {
        XCTAssertEqual(
            MailText.previewText(from: "[cid:logo123] Your invoice is ready"),
    ...
    func testZeroWidthPaddingIsRemoved() {
        ...
        XCTAssertEqual(MailText.previewText(from: padded), "Real content here")
    }

    func testUrlsAreDropped() {
        XCTAssertEqual(
            MailText.previewText(from: "Click https://example.com/very/long/tracking/url to see"),
            "Click to see"
        )
    }
    ...
    func testWhitespaceCollapses() {
        XCTAssertEqual(
            MailText.previewText(from: "Too   many\n\n  spaces\there"),
    ...
    func testStyleBlockContentIsRemoved() {
        ...
        let stripped = MailText.strippingHTML(html)
    ...
    func testCommentsAreRemoved() {
        let stripped = MailText.strippingHTML("<!-- hidden note --><p>Shown</p>")
        XCTAssertFalse(stripped.contains("hidden note"))
        XCTAssertTrue(stripped.contains("Shown"))
    }

    func testEntitiesAreDecoded() {
        XCTAssertEqual(MailText.strippingHTML("<p>Tom &amp; Jerry&#39;s</p>"), "Tom & Jerry's")
    }

    func testStyleStrippingIsCaseInsensitive() {
        let stripped = MailText.strippingHTML("<STYLE>p { color: red; }</STYLE><p>Text</p>")
